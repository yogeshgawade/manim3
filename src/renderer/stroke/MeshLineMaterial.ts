...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // Clip based on visibility: only show where vCounters < visibility
    float clipFactor = step(vCounters, visibility);
    if (clipFactor < 0.5) {
      discard;  // Fully discard clipped fragments
    }
    diffuseColor.a *= clipFactor;
