...
export class MeshLineGeometry extends THREE.BufferGeometry {
  ...
  process(): void {
    ...
    if (!this._attributes || this._attributes.position.count !== this.counters.length) {
      ...
      this._attributes.counters.copyArray(new Float32Array(this.counters))
  ...
  advance({ x, y, z }: THREE.Vector3) {
    const positions = this._attributes.position.array as Float64Array
    const previous = this._attributes.previous.array as Float64Array
    const next = this._attributes.next.array as Float64Array
