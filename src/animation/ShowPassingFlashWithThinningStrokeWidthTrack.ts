/**
 * TaperedFlash animation track - flash with tapered stroke.
 *
 * Creates a flash traveling along a path with stroke width that tapers
 * from full width at the head to thin at the tail.
 */
...
export interface TaperedFlashOptions {
...
/**
 * TaperedFlashTrack — Flash with tapered stroke width.
 */
export class TaperedFlashTrack extends BaseAnimationTrack {
  private flashColor: string;
  private timeWidth: number;
  private flashStrokeWidth: number;
  private minStrokeWidthRatio: number;
  ...
  prepare(): void {
    ...
    for (let i = 0; i < numSegments; i++) {
      const idx = Math.min(i, this.pathPoints.length - 2);
  ...
  interpolate(alpha: number): void {
    ...
    // Update flash segments with tapered stroke width
    ...
    this.flashSegments.forEach((line, i) => {
      ...
      // Calculate progress along the flash (0 = tail, 1 = head)
      const progressAlongFlash = i / (numSegments - 1 || 1);

      // Stroke width taper: min at tail, max at head
      ...
      line.setEnd(endPos);
      line.strokeWidth = this.flashStrokeWidth * strokeWidthMultiplier;
      line.opacity = segmentOpacity;
      line.markDirty();
    });
  }
...
/**
 * Create a TaperedFlash animation track.
 * @param mob The mobject to show flash on
 * @param options Animation options (color, timeWidth, strokeWidth, minStrokeWidthRatio)
 */
export function taperedFlash(
  mob: Mobject,
  options?: TaperedFlashOptions,
): TaperedFlashTrack {
  return new TaperedFlashTrack(mob, options);
}
