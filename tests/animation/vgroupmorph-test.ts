...
mathTex3.position = [4, 2, 0];

// Create MathTex2 at different (right-bottom) position with different equation
const mathTex4 = new MathTex({
  latex: '1234',
  color: RED,
  fontSize: 2,
  strokeWidth: 2,
...
async function runAnimation() {
  ...
  scene.at(0).play(fadeIn(mathTex3, { duration: 1.5, rateFunc: smooth }));
  ...
  scene.at('+=0.5').play(new VGroupMorphTrack(mathTex3, mathTex4, 2, smooth));
  console.log('[VGroupMorphTrack Test] Phase 2: MathTex1 morphs to MathTex2 (different position: 3, -1, 0)');
  scene.at('+=0.5').play(fadeOut(mathTex3, { duration: 1.5, rateFunc: smooth }));
